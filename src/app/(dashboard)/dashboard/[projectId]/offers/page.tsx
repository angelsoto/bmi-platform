...
import { Gift, AlertTriangle, ChevronRight, Trash2, CheckSquare, Square, Edit3 } from "lucide-react";
...
export default function OffersPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Delete ${selected.size} offer${selected.size > 1 ? "s" : ""}?`)) return;
    try {
      const res = await fetch(`/api/projects/${params.projectId}/offers/bulk-delete`, {
        method: "POST",
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          {loading ? (
            ...
            <EmptyState icon={Gift} title="No offers yet" description="Create one above or generate via concept intake." />
          ) : (
            <div className="space-y-2">
              {offers.map((o: any) => (
                <div key={o.id} className="group rounded-md border bg-white shadow-sm hover:shadow-md hover:border-navy-300 transition-all">
                  {selectMode ? (
                    <div className="flex items-center gap-3 p-3">
                      <button onClick={() => toggleSelect(o.id)} className="shrink-0">
                        <div className={`flex h-5 w-5 items-center justify-center rounded border-2 ${selected.has(o.id) ? "border-navy-900 bg-navy-900" : "border-gray-300"}`}>
                          {selected.has(o.id) && <CheckSquare className="h-3.5 w-3.5 text-white" />}
                        </div>
                      </button>
                      <div className="min-w-0 flex-1">
                        <h3 className="text-sm font-medium text-navy-900">{o.name}</h3>
                        <p className="text-xs text-gray-500 truncate">{o.valueProposition}</p>
                      </div>
                    </div>
                  ) : (
                    <Link href={`/dashboard/${params.projectId}/offers/${o.id}`} className="flex items-center gap-3 p-3">
